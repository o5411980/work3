...
$(document).ready(function () {
  ...
  function score_indicate() {
    ...
    let average = sum / subject_points.length;// ヒント：変数「average」に平均値を出して代入しましょう(平均をとりたい数の合計点数(sum) / 全体の個数)。ヒント：全体の個数はlengthメソッドを使って求めます。
    $('#average_indicate').text(average);// 平均点(id=average_indicate)にaverageの値をテキスト挿入。合計点と同じ構文。
  }; //関数score_indicateの閉じタグ
...
//    console.log(averageIndicate)
    ...
    if (averageIndicate >= 80) {
      return "A";
    } else if (averageIndicate >= 60) { //もし「averageIndicate」が60以上なら
      return "B";  // "B"を返します。
    } else if (averageIndicate >= 40) { //もし「averageIndicate」が40以上なら
      return "C";// "C"を返します。
    } else { //もし「averageIndicate」がそれ以外なら
      return "D"   // "D"を返します。
    }
  }; //関数get_achievementの閉じタグ
  ...
  // 最終的なジャッジのロジックを作ります。
  function judgement() { //関数function judge設定
  ...
  $('#btn-declaration').click(function () {
    $('#alert-indicate').remove(); //judgement関数がappendする前に,id=alert-indicateがあれば削除
    judgement(); // judgement関数により最終ジャッジがアラート表示。class=alert-info
  });
});
