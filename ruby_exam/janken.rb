class Player # プレイヤー(自分)に「0~2」を入力させるロジックを書きます。
  def hand     # プレイヤーにじゃんけんの手を選択させる文章を表示させます。
    ...
    # 変数「input_hand」にプレイヤーの入力値を代入します。 # ヒント：getsメソッド
    input_hand = gets.to_i #入力を受け付けて整数に変換
    # 「input_hand」が「0, 1, 2」のいずれかだと繰り返し処理を終了し、それ以外（アルファベットも含む）だと繰り返し処理を継続します。
    while true
      if [0, 1, 2].include?(input_hand) # if 「input_hand」が「0, 1, 2」のいずれかの場合だった場合
        return input_hand
      else # else それ以外の場合
...
end # class Player文のend
# 相手が「0~2」の値をランダムに生成するロジックを書きます。
class Enemy
  def hand
    return rand(0...3)# グー、チョキ、パーの値をランダムに取得する。
  end
end
...
class GameStart
  ...
  def self.jankenpon
    ...
    janken = Janken.new
    next_game = "true"# 変数「next_game」に初期値「true」を代入しましょう。
